package eu.getsoftware.hotelico.service.booking.application.hotel.domain.usecase.notification;
...
/*public*/ class NotificationUseCaseImpl implements NotificationUseCase<InnerHotelEvent>
{
	...
	public void notificateAboutEntityEventWebSocket(CustomerDTO dto, InnerDomainEvent event, String eventContent, long entityId)
	{
		...
		if(InnerHotelEvent.EVENT_LOGO_CUSTOMER_CHANGE_MESSAGE.equals(event) && !allOnlineCustomerIds.contains(dto.getDomainEntityId()))
		{
...
//								simpMessagingTemplate.convertAndSend(AppConfigProperties.SOCKET_CHAT_TOPIC + lastUnreadMessage.getSenderId() + "", chatMessageDto);
			...
			Map<CustomerDomainEntityId, String> lastMessagesForCustomer = new HashMap<>();
			Map<CustomerDomainEntityId, String> lastMessageTimesForCustomer = new HashMap<>();
			Map<CustomerDomainEntityId, Boolean> lastMessageSeenByCustomer = new HashMap<>();
			Map<CustomerDomainEntityId, Boolean> lastMessageDelieveredToCustomer = new HashMap<>();
			...
			for (CustomerDTO nextChatCustomerRootEntity : allChatPartners)
			{
				...
				if (nextLastMessage != null)
				{
					...
					lastMessagesForCustomer.put(nextChatCustomerRootEntity.getDomainEntityId(), message);// + " (" + time + ")");
					lastMessageTimesForCustomer.put(nextChatCustomerRootEntity.getDomainEntityId(), time);
					...
					{
						lastMessageSeenByCustomer.put(nextChatCustomerRootEntity.getDomainEntityId(), nextLastMessage.receiverDomainId().equals(nextChatCustomerRootEntity.getDomainEntityId()) && nextLastMessage.seenByReceiver());
						lastMessageDelieveredToCustomer.put(nextChatCustomerRootEntity.getDomainEntityId(), nextLastMessage.receiverDomainId().equals(nextChatCustomerRootEntity.getDomainEntityId()) && nextLastMessage.deliveredToReceiver());
					}
				}
				//#######################
			}
...
//		List<CheckinRootDomainEntity> allActiveInHotel = checkinService.getActiveByHotelId(hotelActivity.getHotel().getId(), new Date());
		...
		for (CustomerDomainEntityId nextGuestId : notLoggedGuestIds)
		{
			...
			{
				String pushUrlPostfix = "//" + hotelActivity.getHotelId() + "/" + hotelActivity.getDomainEntityId();
