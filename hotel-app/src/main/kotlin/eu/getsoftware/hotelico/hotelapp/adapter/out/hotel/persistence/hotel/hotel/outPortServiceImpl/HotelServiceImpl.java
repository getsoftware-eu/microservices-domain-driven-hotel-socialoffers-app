...
import eu.getsoftware.hotelico.hotelapp.adapter.out.hotel.persistence.hotel.hotel.repository.ActivityRepository;
...
import eu.getsoftware.hotelico.hotelapp.adapter.out.hotel.persistence.hotel.hotel.repository.WallPostRepository;
...
public class HotelServiceImpl implements IHotelService
{
    ...
    @Override
    public Optional<HotelRootEntity> getEntityById(long hotelId) {
    ...
    {
        CustomerRootEntity sender = customerRepository.findById(AppConfigProperties.getTryEntityId(customerId))
...
//				.sorted((object1, object2) -> Integer.compare(object1.getOrderIndex(), object2.getOrderIndex()));
    ...
    @Override
    public double getDistanceKmToHotel(Point2D.Double from, IHotelRootEntity to) {
        return 0;
    }

    @Override
    public void avoidDoubleInitId(List<ICustomerDeal> resultList) {
...
//        }
	...
	public CustomerDealDTO addUpdateDeal(long guestCustomerId, long activityId, CustomerDealDTO dealDto)
	{
        ...
        CustomerRootEntity sender = customerRepository.getOne(AppConfigProperties.getTryEntityId(guestCustomerId));
        ...
        {
            Optional<HotelActivity> activityOptional = getActivityByIdOrInitId(AppConfigProperties.getTryEntityId(activityId), activityId);
...
//                    }
    ...
    public HotelActivityDTO addUpdateHotelActivity(long customerId, HotelActivityDTO hotelActivityDto)
    {
        if(customerId>0)
        {
	        Optional<CustomerRootEntity> creatorOpt = customerRepository.findById(AppConfigProperties.getTryEntityId(customerId));
    ...
    {
        CustomerRootEntity sender = customerService.getEntityById(wallPostDto.getSenderId()).orElseThrow(()-> new RuntimeException("wallpost not found"));
        ...
        HotelRootEntity hotelRootEntity = this.getEntityById(wallPostDto.getHotelId()).orElseThrow(()-> new RuntimeException("wallpost not found"));
...
//        activityDto.setLikeCounter(5);
    ...
    public List<HotelActivityDTO> getHotelActivitiesBySenderAndHotelId(long senderId, long hotelId)
    {
        ...
        CustomerRootEntity customerEntity = customerService.getEntityById(AppConfigProperties.getTryEntityId(senderId));
...
//			String hotelPushIds = hotel.getGuestPushIds();
    ...
    @Override
    public IHotelRootEntity findByCurrentHotelAccessCodeAndActive(String hotelCode, boolean active) {
        return null;
    }
    ...
    {
        CustomerDTO dto = customerService.getById(AppConfigProperties.getTryEntityId(customerId), 0);
