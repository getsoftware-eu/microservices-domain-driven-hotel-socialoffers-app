...
import eu.getsoftware.hotelico.service.LoginHotelicoService;
...
//		FacebookProfile profile2 = facebook.userOperations().getUserProfile();
		...
		CustomerDTO foundFacebookCustomer = customerService.getByFacebookId(facebookUserId);
		...
		else if(foundFacebookCustomer!=null && tempfoundCacheSocialCustomer!=null)
		{
			foundFacebookCustomer = loginService.checkBeforeLoginProperties(tempfoundCacheSocialCustomer, foundFacebookCustomer);
		}
		
		// ########################################
		
		if(foundFacebookCustomer==null || foundFacebookCustomer.getFirstName()==null)
		{
			if(foundFacebookCustomer==null)
			{
				foundFacebookCustomer = new CustomerDTO();
			}

			if (userProfile.getFirstName() != null)
			{
				foundFacebookCustomer.setFirstName(userProfile.getFirstName());
				foundFacebookCustomer.setLastName(userProfile.getLastName());
			}
			else if(profile!=null && profile.getFirstName()!=null)
			{
				...
				if(fullName!=null)
				{
					...
					foundFacebookCustomer.setFirstName(firstName);
					foundFacebookCustomer.setLastName(lastName);
				}
			}

			if(facebookEmail!=null)
				foundFacebookCustomer.setEmail(userProfile.getEmail());			
			
			if(profile!=null && Locale.GERMAN.equals(profile.getLocale()))
			{
				if(!ControllerUtils.isEmptyString(profile.getLocale().getLanguage()))
				{
					foundFacebookCustomer.setPrefferedLanguage(profile.getLocale().getLanguage().toLowerCase());
				}
			}
			...
			if(profile!=null && profile.getGender()!=null)
			{
				...
				foundFacebookCustomer.setSex(gender);
			}
			else
				foundFacebookCustomer.setSex("m");
			...
			if(connection.getImageUrl()!=null)
				foundFacebookCustomer.setProfileImageUrl(connection.getImageUrl());

			foundFacebookCustomer.setLogged(true);

			foundFacebookCustomer = customerService.addFacebookCustomer(foundFacebookCustomer, facebookUserId);
		...
		if(foundFacebookCustomer!=null)
		{
			...
			if(foundFacebookCustomer.getEmail()==null || foundFacebookCustomer.getEmail().isEmpty())
			{
				foundFacebookCustomer.setEmail("");
			}

			loginService.setLogged(foundFacebookCustomer.getId(), true);
			//			try
			//			{
			//				Customer entity = customerService.getEntityById(foundFacebookCustomer.getId());
			//			
			//				if(entity!=null){
			//					entity.setLogged(true);
			//					entity.updateLastSeenOnline();
			...
			//				;
			//			}
			session.setAttribute(ControllerUtils.SESSION_CUSTOMER_ID, foundFacebookCustomer.getId());
			session.setAttribute(ControllerUtils.SESSION_CUSTOMER, foundFacebookCustomer);
