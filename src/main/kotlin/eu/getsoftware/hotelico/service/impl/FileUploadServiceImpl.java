...
//						}
					...
					case "hotel":
					{
						...
						if(hotel==null)
						{
							...
							Optional<Hotel> hotelOptional = hotelRepository.findByCreationTimeAndActive(hotelCreationTime, true).stream().findAny();
						...
						if (hotel != null)
						{
							Path fileAdded = this.addResizeImage(hotel, absolutePath, "logo", fileType, file.getInputStream());
							Path previewAdded = this.addResizeImage(hotel, absolutePath, "preview", fileType, file.getInputStream());
							
							if (fileAdded != null)
							{
								hotel.setPictureUrl(ControllerUtils.getRelativePath(fileAdded));
								hotel.setConsistencyId(new Date().getTime());
							}
							
							if (previewAdded != null)
							{
								hotel.setPreviewPictureUrl(ControllerUtils.getRelativePath(previewAdded));
								hotel.setConsistencyId(new Date().getTime());
							}
							hotelRepository.saveAndFlush(hotel);

							//TODO Eugen: send hotel image notification
							notificationService.notificateAboutEntityEvent(customerService.convertCustomerToDto(sender, hotel.getId()), HotelEvent.EVENT_LOGO_HOTEL_CHANGE_MESSAGE, hotel.getPictureUrl(), hotel.getId());
						}
	...
	public CustomerDTO getEntityImage(long requesterId, String entityType, long entityId)
	{
		...
		switch (entityType.toLowerCase())
		{
			...
			case "customer":
			{
				...
				Customer customer = customerRepository.getOne(entityId);
...
//				List<HotelActivity> activities = activityRepository.getByInitId(entityId);
			...
			case "hotel":
			{
				...
				if (hotel != null)
				{
					pictureUrl = hotel.getPictureUrl();				
				}
				break;
			}
		}
