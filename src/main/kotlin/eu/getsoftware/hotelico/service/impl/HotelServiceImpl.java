...
import eu.getsoftware.hotelico.repository.DealRepository;
import eu.getsoftware.hotelico.repository.CheckinRepository;
import eu.getsoftware.hotelico.repository.CustomerRepository;
import eu.getsoftware.hotelico.repository.ActivityRepository;
import eu.getsoftware.hotelico.repository.HotelRepository;
import eu.getsoftware.hotelico.repository.WallPostRepository;
import eu.getsoftware.hotelico.service.CacheService;
import eu.getsoftware.hotelico.service.CheckinService;
...
public class HotelServiceImpl implements HotelService
{
    ...
    public List<HotelDTO> getHotels() {
        List<Hotel> list = hotelRepository.findByVirtualAndActive(false, true);
        List<HotelDTO> out = new ArrayList<HotelDTO>();
        for (Hotel hotel : list) {
            
            boolean ignoreDemoHotel = ControllerUtils.HIDE_DEMO_HOTEL_FROM_HOTEL_LIST && ControllerUtils.HOTEL_DEMO_CODE.equalsIgnoreCase(hotel.getCurrentHotelAccessCode());

            if(hotel.isActive() && !ignoreDemoHotel)
            {
                out.add(convertHotelToDto(hotel));
            }
    ...
    public List<HotelDTO> getHotelCities(long requesterId) {
        ...
        Customer requester = requesterId>0? customerRepository.getOne(requesterId): null;
        ...
        for (Hotel hotel : list) {
            ...
            if(hotel.isActive() && !hotel.isVirtual() && !ignoreDemoHotel && (!citiesList.contains(hotel.getCity())))
            {
                citiesList.add(hotel.getCity());
                HotelDTO hotelDto = convertHotelToDto(hotel);
                
                if(requester!=null && hotel.getLatitude()>-90 && requester.getLatitude()>-90)
                {
                    double kms = getDistanceKmToHotel(new Point2D.Double(requester.getLatitude(), requester.getLongitude()), hotel);
    ...
    public List<HotelDTO> getHotelsByCity(long requesterId, String filterCity) throws HotelException
    {
        ...
        Optional<Customer> requesterOptional = customerRepository.findById(requesterId);
        ...
        for (Hotel hotel : list) {
            ...
            if(hotel.isActive() && !hotel.isVirtual() && !ignoreDemoHotel && (filterCity==null || filterCity.equals(hotel.getCity())))
            {
                HotelDTO hotelDto = convertHotelToDto(hotel);
                if(requesterOptional.isPresent() && hotel.getLatitude()>-90 && requesterOptional.get().getLatitude()>-90)
                {
	                Customer requester = requesterOptional.get();
                    double kms = getDistanceKmToHotel(new Point2D.Double(requester.getLatitude(), requester.getLongitude()), hotel);
                    hotelDto.setKmFromMe(kms);
                }
                out.add(hotelDto);
    ...
    @Override
    public Hotel getEntityById(long hotelId) {
    ...
    public HotelDTO getHotelByCode(String hotelCode) {
        ...
        Hotel hotel = hotelRepository.findByCurrentHotelAccessCodeAndActive(hotelCode, true);
    ...
    {
        List<Customer> participantsList = wallPostRepository.getParticipantsByHotelId(hotelId, new Date());

        List<CustomerDTO> out = new ArrayList<CustomerDTO>();

        for (Customer nextParticipant : participantsList) {
    ...
    public HotelActivityDto addActivityAction(long customerId, long activityId, String action)
    {
        ...
        Customer sender = customerRepository.getOne(ControllerUtils.getTryEntityId(customerId));
...
//        HotelActivity activity = activityRepository.getByInitId(activityId);
        ...
        switch (action)
        {
            case "like":
            {
                Set<Customer> likedBy = activity.getLikedCustomers();
                likedBy.add(sender);
                activity.setLikedCustomers(likedBy);
			...
			case "subscribe":
            {
                Set<Customer> subscribeBy = activity.getSubscribeCustomers();
				subscribeBy.add(sender);
                activity.setSubscribeCustomers(subscribeBy);
                activityRepository.saveAndFlush(activity);
...
//				int hotelId = activity.getHotel().getId();
				...
				ReorderAction reorderAction = ReorderAction.parseByTitle(action);
	...
	public void reorderActivitiesInHotel(ReorderAction action, HotelActivity activity)
	{
		...
		List<HotelActivity> hotelActivities = activityRepository.getAllTimesByHotelId(activity.getHotel().getId());
...
//				.sorted((object1, object2) -> Integer.compare(object1.getOrderIndex(), object2.getOrderIndex()));
    ...
    {
        List<Hotel> hotels = hotelRepository.findActiveGpsHotels();

        for(Hotel nextHotel: hotels)
        {
            double kmFrom = this.getDistanceKmToHotel(latLonPoint, nextHotel);

            if(kmFrom<100)
            {
                return nextHotel.getCity();
            }
    ...
    @Override
    public double getDistanceKmToHotel(Point2D.Double from, Hotel hotel)
    {
        if(hotel.getLatitude()<=-90)
        {
            return -1;
        }

        double earthRadius = 3958.75; // miles (or 6371.0 kilometers)
        double dLat = Math.toRadians(hotel.getLatitude()-from.getX());
        double dLng = Math.toRadians(hotel.getLongitude()-from.getY());
        double sindLat = Math.sin(dLat / 2);
        double sindLng = Math.sin(dLng / 2);
        double a = Math.pow(sindLat, 2) + Math.pow(sindLng, 2)
                * Math.cos(Math.toRadians(from.getX())) * Math.cos(Math.toRadians(hotel.getLatitude()));
...
//        }
        ...
        Customer requester = customerRepository.getOne(customerEntityId);
	...
	public CustomerDealDto addUpdateDeal(long guestCustomerId, long activityId, CustomerDealDto dealDto)
	{
        ...
        Customer sender = customerRepository.getOne(ControllerUtils.getTryEntityId(guestCustomerId));
...
//        }
		...
		Customer sender = customerRepository.getOne(customerEntityId);
...
//                    }
        ...
        if(resultDealDto!=null)
        {
            ...
            Customer staff = checkinService.getStaffbyHotelId(resultDealDto.getHotelId());
    ...
    public CustomerDeal createDeal(Customer sender, long guestCustomerId, HotelActivity activity)
    {
    ...
    public HotelDTO getHotelById(long hotelId) {
        ...
        if(hotelId==-999)
        {
			List<Hotel> virtualHotels = hotelRepository.findByVirtualAndActive(true, true);
			
			if(!virtualHotels.isEmpty())
			{
				hotel = virtualHotels.get(0);
			}
        }
        ...
        HotelDTO out = hotel==null? null : convertHotelToDto(hotel);
    ...
    public HotelDTO addHotel(HotelDTO hotelDto) {
		...
		{
			List<Hotel> hotelsWithSameMail = hotelRepository.findByMailAndActive(hotelDto.getEmail(), true);
        ...
        hotel.setConsistencyId(new Date().getTime());

        return convertHotelToDto(hotelRepository.saveAndFlush(hotel));
    }
    ...
    public HotelActivityDto addUpdateHotelActivity(long customerId, HotelActivityDto hotelActivityDto)
    {
        if(customerId>0)
        {
            Customer creator = customerRepository.getOne(ControllerUtils.getTryEntityId(customerId));
        ...
        if(hotelActivity == null)
        {
            hotelActivityDto = createHotelActivity(hotelActivityDto);
        }
        else
        {
            hotelActivityDto = updateHotelActivity(hotelActivityDto);
        }
    ...
    @Override
    public HotelActivity getActivityByIdOrInitId(long id, long initId)
    {
        HotelActivity hotelActivity = null;
       
        if(initId>0)
        {
            List<HotelActivity> hotelActivities = activityRepository.findByInitIdAndActive(initId, true);
            hotelActivity = hotelActivities.isEmpty()? hotelActivity : hotelActivities.get(0);
        }    
        
        if(hotelActivity==null)
        {
            hotelActivity = activityRepository.getOne(id);
        }
    ...
    @Override
    public CustomerDeal getDealByIdOrInitId(long id, long initId)
    {
        CustomerDeal customerDeal = null;
       
        if(initId>0)
        {
            List<CustomerDeal> customerDeals = dealRepository.getByInitId(initId);
            
            if(!customerDeals.isEmpty())
            {
                customerDeal = customerDeals.get(0);
            }
        }    
        
        if(customerDeal==null)
        {
            customerDeal = dealRepository.getOne(id);
        }
    ...
    private HotelActivityDto createHotelActivity(HotelActivityDto hotelActivityDto)
    {
		...
		{
			hotel = hotelRepository.getOne(hotelActivityDto.getHotelId());
		}
    ...
    public HotelActivity fillActivityFromDto(HotelActivity hotelActivity, HotelActivityDto hotelActivityDto)
    {
		...
		{
			hotelActivity.setValidTo(new DateTime().plusDays(7).toDate());
		}
    ...
    {
        Customer sender = customerService.getEntityById(wallPostDto.getSenderId());
        ...
        Hotel hotel = this.getEntityById(wallPostDto.getHotelId());

        HotelWallPost newMessage = new HotelWallPost(wallPostDto.getMessage(), sender, hotel);
        ...
        if(ControllerUtils.HOTEL_DEMO_CODE.equalsIgnoreCase(hotel.getCurrentHotelAccessCode()))
        {
            newMessage.setValidUntil(new DateTime().plusMonths(6).toDate());
        }
        else {
            newMessage.setValidUntil(new DateTime().plusDays(3).toDate());
        }
...
//    }
    ...
    public HotelDTO updateHotel(HotelDTO hotelDto) {
        ...
        if(hotel!=null)
        {
            ...
            hotel.setInfo(hotelDto.getInfo());
            hotel.setName(hotelDto.getName());
            hotel.setEmail(hotelDto.getEmail());
            hotel.setCurrentHotelAccessCode(hotelDto.getCurrentHotelAccessCode());
            hotel.setCity(hotelDto.getCity());
            hotel.setStreet(hotelDto.getStreet());
            hotel.setHouse(hotelDto.getHouse());
            hotel.setPostalCode(hotelDto.getPostalCode());
            hotel.setFax(hotelDto.getFax());
...
//            hotel.setPictureUrl(hotelDto.getPictureUrl());
            ...
            {
                hotel.setCreationTime(hotelDto.getCreationTime());
            }   
			
			if(!ControllerUtils.isEmptyString(hotelDto.getWellcomeMessage()))
            {
                hotel.setWellcomeMessage(hotelDto.getWellcomeMessage());
            }
    ...
    private HotelDTO convertHotelToDto(Hotel hotel)
    {
        ...
        try{
            activityNumber = activityRepository.getTimeValidCounterByHotelId(hotel.getId(), new Date());
            customerNumber = checkinRepository.getFullCheckinCountExcludingStaffByHotelId(hotel.getId(), new Date());
        }
        catch (Exception e){
            ;//TODO Eugen.
        }
        ...
        dto.setCreationTime(hotel.getCreationTime());

		Map<Long, String> anonymeGuests = getNotLoggedGuestPushIdsByHotel(hotel);
    ...
    public HotelActivityDto updateHotelActivity(HotelActivityDto hotelActivityDto)
    {
        ...
        if(hotelActivity!=null)
        {
            hotelActivity = fillHotelFromDto(hotelActivityDto, hotelActivity);
	...
	@Override
	public Map<Long,String> getNotLoggedGuestPushIdsByHotel(Hotel hotel)
	{
		...
		String hotelPushIds = hotel.getGuestPushIds()!=null?  hotel.getGuestPushIds() : "";
...
//		}
		...
		List<Hotel> allActiveHotels = hotelRepository.findByVirtualAndActive(false, true);
		
		for (Hotel nextHotel : allActiveHotels)
		{
			String hotelUnsubscribePushIds = nextHotel.getUnsubscribeNotificationPushIds()!=null?  nextHotel.getUnsubscribeNotificationPushIds() : "";
			
			String[] guestPushIds = hotelUnsubscribePushIds.split("<#>");
			...
			for (String guestPushIdMapping : guestPushIds)
			{
				...
				if (pushMapping.length > 1)
				{
					String nextUnsubscribeGuestId = pushMapping[0];
					String nextUnsubscribePushId = pushMapping[1];
					
					//				if (!loggedGuestPushIds.contains(nextPushId) && !ControllerUtils.isEmptyString(nextPushId))
					{
						unsubscribeGuestsToPushId.put(Long.parseLong(nextUnsubscribeGuestId), nextUnsubscribePushId);
					}
				}
			}
		}
...
//        hotelActivity = modelMapper.map(hotelActivityDto, HotelActivity.class);
        ...
        hotelActivity.setValidTo(hotelActivityDto.getValidTo() != null && !hotelActivityDto.getLastMinute() ? hotelActivityDto.getValidTo() : hotelActivityDto.getLastMinute() ? new DateTime().plusDays(1).withHourOfDay(4).toDate() : new DateTime().plusDays(7).toDate());
    ...
    public WallPostDto updateWallPost(WallPostDto wallPostDto)
    {
        ...
        HotelWallPost updateWallPost = null;
       
        if(!wallPosts.isEmpty())
        {
            updateWallPost = wallPosts.get(0);
...
//            requester.getSeenActivities().clear();
            ...
            if(requester==null && requesterId>0)
            {
...
//                requester.getSeenActivities().add(hotelActivity);
    ...
    @Override
    public HotelActivityDto convertActivityToDto(HotelActivity hotelActivity, Customer requester)
    {
        if(hotelActivity == null)
        {
            return null;
        ...
        activityDto.setLikeCounter(hotelActivity.getLikedCustomers().size());
        activityDto.setSubscribeCounter(hotelActivity.getSubscribeCustomers().size());
...
//        activityDto.setSubscribeByMe(hotelActivity.getSubscribeCustomers().contains(requester));
        ...
        if(hotel!=null)
        {
            activityDto.setHotelId(hotel.getId());
            activityDto.setHotelName(hotel.getName());
            activityDto.setHotelCity(hotel.getCity());

            try
            {
                otherActivityNumber = activityRepository.getTimeValidCounterByHotelId(hotel.getId(), new Date());
                hotelCustomerNumber = checkinRepository.getFullCheckinCountExcludingStaffByHotelId(hotel.getId(), new Date());
            }
            catch (Exception e)
            {
                ;//TODO Eugen.
...
//        activityDto.setLikeCounter(5);
    ...
    public List<HotelActivityDto> getHotelActivitiesBySenderAndHotelId(long senderId, long hotelId)
    {
        ...
        Customer customer = customerService.getEntityById(ControllerUtils.getTryEntityId(senderId));
        
        if(customer!=null && customer.isHotelStaff() || customer.isAdmin())
        {
            list = activityRepository.getAllTimesByHotelId(hotelId);
        }
        else{
        ...
        for (HotelActivity hotelActivity : list) {
            ...
            HotelActivityDto activityDto = convertActivityToDto(hotelActivity, customer);
            out.add(activityDto);
        }
        return out;
    }
...
//        dto.setId(wallPost.getId());

        Customer sender = wallPost.getSender();
        ...
        if(wallPost.getSpecialWallContent()!=null)
        {
            ...
            if(split.length>1)
            {
                ...
                if(key.equalsIgnoreCase("customerId"))
                {
                    ...
                    if(customer!=null)
                    {
                        dto.getSpecialContent().put("name", customer.getFirstName() + " " + customer.getLastName());
                        dto.getSpecialContent().put("pictureUrl", customerService.getCustomerAvatarUrl(customer));
                    }
	...
	public CustomerDTO addGuestAction(long guestCustomerId, String action, Long hotelId, CustomerDTO guestDto)
	{
		...
		if ((hotelId == null || hotelId <= 0) && guestDto.getSystemMessages().get("actionHotelId")!=null)
		{
			hotelId = Long.parseLong(guestDto.getSystemMessages().get("actionHotelId"));
		}
		...
		{
			Map<Long, String> getNotLoggedGuestPushIds = getNotLoggedGuestPushIdsByHotel(hotel);
			...
			Long dtoGuestCustomerId = Long.parseLong(guestDto.getSystemMessages().get("guestCustomerId"));
			String guestPushId = guestDto.getSystemMessages().get(ControllerUtils.PUSH_CHROME_ID);
...
//			if (getNotLoggedGuestPushIds.containsKey(dtoGuestCustomerId))
			...
			hotel.setGuestPushIds(hotelGuestPushIds);
			hotelRepository.saveAndFlush(hotel);
...
//			String hotelPushIds = hotel.getGuestPushIds();
		...
		else if("unsubscribeHotelActivityNotifications".equalsIgnoreCase(action))
		{
			...
			String unsubscribeNotificationPushIds = hotel.getUnsubscribeNotificationPushIds();
			...
			hotel.setUnsubscribeNotificationPushIds(unsubscribeNotificationPushIds);
			hotelRepository.saveAndFlush(hotel);
		}
    ...
    public ResponseDTO deleteHotel(long hotelId, long customerId) {
        ...
        List<Customer> hotelStaff = checkinRepository.getStaffByHotelId(hotelId);

        for (Customer nextStaff: hotelStaff)
        {
    ...
    public ResponseDTO deleteHotelActivity(long customerId, long activityId)
    {
        ...
        return new ResponseDTO("Activity error", true);
