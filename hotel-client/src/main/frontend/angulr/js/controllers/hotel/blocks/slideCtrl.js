...
.controller('SlideCtrl', [ '$scope', '$filter', 
    ...
    function getSlide(target, style) {
        ...
        if(!$scope.labels[style])
        {
            $scope.labels[style] = [];
        }
        
        if(!$scope.labels[style][(i + 1)])
        {
            $scope.labels[style][(i + 1)] = $filter('translate')('slide.'+style+(i + 1));
        }
        
        return {
            id: (i + 1),
            label: $scope.labels[style][(i + 1)] ,
   ...
   function showCarouselLabels()
   {
       var carouselLabelContents = document.getElementsByClassName("carousel-top-label");

       if (carouselLabelContents && carouselLabelContents.length > 0) {
           for (var i = 0; i < carouselLabelContents.length; i++) {
               var nextElt = carouselLabelContents[i];
               nextElt.style.visibility = "visible ";
           }
       }
   }
                       
   function initSlides(showTopic, slidesCount){
       ...
       // 2nd ngRepeat demo
       $scope.slides2 = [];
       addSlides($scope.slides2, showTopic, slidesCount);
       ...
       var slideImages = [];
       addSlides(slideImages, showTopic, slidesCount);
       ...
       setTimeout(showCarouselLabels, 500);

   }
              
   if($scope.carousel.topic && $scope.carousel.length)                    
   {
       initSlides($scope.carousel.topic, $scope.carousel.length);
   }
