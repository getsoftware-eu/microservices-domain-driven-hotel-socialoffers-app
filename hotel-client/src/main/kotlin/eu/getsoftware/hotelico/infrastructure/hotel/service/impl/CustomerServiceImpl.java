...
import eu.getsoftware.hotelico.infrastructure.hotel.repository.LanguageRepository;
import eu.getsoftware.hotelico.infrastructure.hotel.service.CacheService;
import eu.getsoftware.hotelico.infrastructure.hotel.service.ChatService;
...
import eu.getsoftware.hotelico.infrastructure.hotel.service.MailService;
...
public class CustomerServiceImpl implements CustomerService
{
    ...
    public List<CustomerDTO> getCustomers() {
        List<CustomerEntity> list = customerRepository.findByActive(true);
        List<CustomerDTO> outList = new ArrayList<CustomerDTO>();
        for (CustomerEntity nextCustomerEntity : list) {
            
            //TODO Eugen: get all customer hotelId with one query!
            long hotelId = getCustomerHotelId(nextCustomerEntity.getId());
            
            CustomerDTO nextDto = convertCustomerToDto(nextCustomerEntity, hotelId);
...
//            outList.add(updateOwnDtoCheckinInfo(nextDto));
    ...
    public CustomerDTO getById(long requesterId, long dtoRequesterId) {
        ...
        CustomerEntity customerEntity = customerRepository.getOne(requesterId);
        ...
        if(requesterId == dtoRequesterId){
            ...
            if(dtoRequesterId >0)
            {
                CustomerEntity requester = getEntityById(dtoRequesterId);
    ...
    @Override
    public CustomerEntity getEntityById(long customerId) {
...
//        return updateOwnDtoCheckinInfo(dto);
    ...
    public CustomerDTO synchronizeCustomerToDto(CustomerDTO dto)
    {
        ...
        {
            CustomerEntity customerEntity = customerRepository.getOne(dto.getId());
...
//            int hotelId = getCustomerHotelId(customer.getId());
    ...
    @Override
    public List<CustomerEntity> getAllOnline()
    {
    ...
    @Override
    public List<CustomerEntity> getAllIn24hOnline()
    {
    ...
    public CustomerDTO updateCustomer(CustomerDTO customerDto, int updatorId) {
        CustomerEntity customerEntity = customerRepository.getOne(customerDto.getId());
        ...
        if(customerDto.getSystemMessages()!=null && !customerDto.getSystemMessages().isEmpty())
        {
           ...
           {
               //eugen2022
               customerEntity.getCustomerDetails().setLatitude(Double.parseDouble(customerDto.getSystemMessages().get("latitude")));
			   customerEntity.getCustomerDetails().setLongitude(Double.parseDouble(customerDto.getSystemMessages().get("longitude")));
           }
        ...
        if(customerEntity !=null)
        {
            ...
            customerEntity.setOriginalCity(customerDto.getOriginalCity());
            customerEntity.setJobTitle(customerDto.getJobTitle());
            customerEntity.setJobDescriptor(customerDto.getJobDescriptor());
            ...
            customerEntity.setAllowHotelNotification(customerDto.getAllowHotelNotification());
            ...
            customerEntity.setPrefferedLanguage(customerDto.getPrefferedLanguage());
            customerEntity.setProfileImageUrl(customerDto.getProfileImageUrl());
            
            //EUGEN: only disable(true)!!! enable is not allowed!
			if(customerDto.getHideCheckinPopup())
			{
				customerEntity.setHideCheckinPopup(customerDto.getHideCheckinPopup());
			}
			
			if(customerDto.getHideChromePushPopup())
			{
				customerEntity.setHideChromePushPopup(customerDto.getHideChromePushPopup());
			}
            
            //EUGEN: only disable(true)!!! enable is not allowed!
            if(customerDto.getHideHotelListPopup())
			{
				customerEntity.setHideHotelListPopup(customerDto.getHideHotelListPopup());
			}
            
            //EUGEN: only disable(true)!!! enable is not allowed!
            if(customerDto.getHideWallPopup())
			{
				customerEntity.setHideWallPopup(customerDto.getHideWallPopup());
			}
...
//        int hotelId = getCustomerHotelId(customerDto.getId());
	...
	@Override
    public boolean relocateGuestDealsToLoggedCustomer(CustomerEntity customerEntity, Long guestCustomerId)
    {
...
//            next.setGuestCustomerId(0);
    ...
    private void fillCustomerFromDto(CustomerDTO customerDto, CustomerEntity customerEntity)
    {
...
//            customer.setBirthday(birthdayDate.toInstant().atZone(ZoneId.systemDefault()).toLocalDate());
        ...
        {
            customerEntity.setProfileImageUrl("");
        }
        
    }
    
    @Override
    public CustomerDTO convertCustomerToDto(CustomerEntity customerEntity, long hotelId){
        return convertCustomerToDto(customerEntity, hotelId, false, null);
    }
    
    @Override
    public CustomerDTO convertCustomerToDto(CustomerEntity customerEntity, boolean fullSerialization, CustomerHotelCheckin validCheckin){
        return convertCustomerToDto(customerEntity, validCheckin.getHotel().getId(), fullSerialization, validCheckin);
    }
    
    @Override
    public CustomerDTO convertMyCustomerToFullDto(CustomerEntity customerEntity){
        return convertCustomerToDto(customerEntity, 0, true, null);
    }
    
    
//    @Override
    private CustomerDTO convertCustomerToDto(CustomerEntity customerEntity, long hotelId, boolean fullSerialization, CustomerHotelCheckin validCheckin)
    {
        if(customerEntity ==null) 
        {
            return null;
...
//      if(hotelId>=0)
     ...
     * @return
     */
    private CustomerDTO fillDtoProfileInfo(CustomerEntity customerEntity, CustomerDTO dto, boolean fullSerialization)
    {
        if(customerEntity !=null && customerEntity.getLanguageSet()!=null)
        {
            ...
            dto.setOnline(ControllerUtils.isCustomerOnline(customerEntity));
            ...
            if(fullSerialization) //SHOW PRIVATE INFORMATION
            {
                ...
                {
                    dto.setBirthdayTime(customerEntity.getBirthday().getTime()+"");
                }
    ...
    @Override
    public String getCustomerAvatarUrl(CustomerEntity customerEntity)
    {
...
//        Customer onlineCustomer = customerRepository.getOne(sessionCustomerId);
    ...
    public CustomerDTO getByLinkedInId(String linkedInId){
        List<CustomerEntity> customerEntities = customerRepository.findByLinkedInIdAndActive(linkedInId, true);
...
//        int hotelId = getCustomerHotelId(customer.getId());
    ...
    public CustomerDTO getByFacebookId(String facebookId){
        List<CustomerEntity> customerEntities = customerRepository.findByFacebookIdAndActive(facebookId, true);
...
//        return updateOwnDtoCheckinInfo(out);
    ...
    public Set<CustomerDTO> getByCity(long customerId, String city)
    {
        ...
        for (CustomerEntity nextCustomerEntity : allCustomerEntities)
        {
            if(nextCustomerEntity.isActive() && (nextCustomerEntity.getCity()!=null && nextCustomerEntity.getCity().equals(city) || city==null && nextCustomerEntity.getCity()==null))
            {
                //TODO Eugen: create global convert method to dto...
                long hotelId = getCustomerHotelId(nextCustomerEntity.getId());
                CustomerDTO dto = convertCustomerToDto(nextCustomerEntity, hotelId);
...
//        }
        ...
        {
            CustomerEntity nextCustomerEntity = nextCheckin.getCustomer();

            //eugen: filter guest accounts not active more than 1 day
            if(nextCustomerEntity.isGuestAccount() && nextCustomerEntity.getLastSeenOnline()!=null && nextCustomerEntity.getLastSeenOnline().before(ControllerUtils.convertToDate(LocalDateTime.now().minusDays(ControllerUtils.AWAY_GUEST_DAYS_TIMEOUT))))
            {
                if(ControllerUtils.SET_AWAY_GUEST_INACTIVE)
                {
                    nextCustomerEntity.setActive(false);
                }

                if(ControllerUtils.SET_AWAY_GUEST_CHECKOUT)
                {
                    //remove guest checkin
                    List<CustomerHotelCheckin> guestCheckin = checkinRepository.getActiveByCustomerId(nextCustomerEntity.getId(), new Date());
                ...
                customerRepository.save(nextCustomerEntity);
                continue;
            }
            
            if(nextCustomerEntity.isHotelStaff())
            {
                continue;
            }
            
            CustomerDTO dto = convertCustomerToDto(nextCustomerEntity, true, nextCheckin);
        ...
        {
            List<CustomerEntity> staffs = checkinRepository.getStaffByHotelId(hotelId);
            
            for(CustomerEntity nextCustomerEntity : staffs)
            {
                CustomerDTO dto = convertCustomerToDto(nextCustomerEntity, hotelId);
    ...
    @Override
    public CustomerDTO setDtoLastMessageWithRequester(CustomerEntity requester, CustomerDTO dto)
    {
    ...
    @Override
    public CustomerEntity addGetAnonymCustomer()
    {
        List<CustomerEntity> anonymes = customerRepository.getAnonymeCustomer();
       
        CustomerEntity anonym = null;
                
        if(anonymes.isEmpty())
        {
            anonym = new CustomerEntity("[anonym]", "[anonym]");
