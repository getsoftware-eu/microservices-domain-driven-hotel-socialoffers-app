...
interface HotelRepository: JpaRepository<eu.getsoftware.hotelico.hotel.model.Hotel, Long> {
	...
	fun findByCurrentHotelAccessCodeAndActive(currentHotelAccessCode: String, active: Boolean = true): eu.getsoftware.hotelico.hotel.model.Hotel
	...
	fun findByVirtualAndActive(virtual: Boolean = false, active: Boolean = true): List<eu.getsoftware.hotelico.hotel.model.Hotel>
	
	@Query(FIND_VIRTUAL_HOTEL_CODE_QUERY)
	fun getVirtualHotelCode(): String
	
	//@Query(FIND_HOTEL_BY_CREATION_TIME_QUERY)
	fun findByCreationTimeAndActive(creationTime: Long, active: Boolean = true): List<eu.getsoftware.hotelico.hotel.model.Hotel>
	
	@Query(FIND_DEMO_HOTEL_ID_QUERY)
	fun getDemoHotelId(): Int

	@Query(FIND_ACTIVE_GPS_HOTELS_QUERY)
	fun findActiveGpsHotels(): List<eu.getsoftware.hotelico.hotel.model.Hotel>
	
//	@Query(FIND_HOTEL_BY_MAIL_QUERY)
	fun findByEmailAndActive(email: String, active: Boolean = true): List<eu.getsoftware.hotelico.hotel.model.Hotel>
}
