...
import eu.getsoftware.hotelico.hotel.dto.HotelActivityDto;
...
import eu.getsoftware.hotelico.hotel.model.HotelWallPost;
...
import eu.getsoftware.hotelico.hotel.repository.ActivityRepository;
...
import eu.getsoftware.hotelico.hotel.utils.ReorderAction;
...
public class HotelServiceImpl implements HotelService
{
    ...
    public List<HotelDTO> getHotelCities(long requesterId) {
        ...
        for (Hotel hotel : list) {
            ...
            if(hotel.isActive() && !hotel.isVirtual() && !ignoreDemoHotel && (!citiesList.contains(hotel.getCity())))
            {
                ...
                if(requester!=null && hotel.getLatitude()>-90 && requester.getCustomerDetails().getLatitude()>-90)
                {
                    double kms = getDistanceKmToHotel(new Point2D.Double(requester.getCustomerDetails().getLatitude(), requester.getCustomerDetails().getLongitude()), hotel);
    ...
    public List<HotelDTO> getHotelsByCity(long requesterId, String filterCity) throws HotelException
    {
        ...
        for (Hotel hotel : list) {
            ...
            {
                HotelDTO hotelDto = convertHotelToDto(hotel);
                if(requesterOptional.isPresent() && hotel.getLatitude()>-90 && requesterOptional.get().getCustomerDetails().getLatitude()>-90)
                {
...
//                    }
    ...
    public HotelDTO addHotel(HotelDTO hotelDto) {
		...
		{
			List<Hotel> hotelsWithSameMail = hotelRepository.findByEmailAndActive(hotelDto.getEmail(), true);
